    ...
    return new Promise((resolve) => {
      try {
        ...
        socket.onerror = (error) => {
          console.error("[LocalWS] WebSocket-fel vid anslutning:", error);
          this.setStatus("error");
          resolve();
        };
        socket.onclose = (event?: CloseEvent) => {
          if (event && !event.wasClean && event.code !== 1000) {
            console.warn(`[LocalWS] Lokal WebSocket-brygga kopplades ner oväntat (kod: ${event.code}, orsak: ${event.reason || "okänd"})`);
          } else {
            console.log("[LocalWS] Lokal WebSocket-brygga stängdes normalt");
          }
