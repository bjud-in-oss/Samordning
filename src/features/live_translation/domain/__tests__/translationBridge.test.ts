...
describe("TranslationBridge Specifications", () => {
  ...
  it("använder dynamisk tokenProvider och hämtar färsk token vid anslutning och hot-swap", async () => {
    let callCount = 0;
    const tokenProvider = vi.fn(async () => { callCount++; return `auth_tokens/ephemeral_token_v${callCount}`; });
    const bridge = createBridge(tokenProvider, "de");
    await bridge.connect();
    expect(tokenProvider).toHaveBeenCalledTimes(1);
    expect(bridge.apiKey).toBe("auth_tokens/ephemeral_token_v1");
    vi.advanceTimersByTime(10);
    const ws1 = MockWebSocket.instances[0]!;
    expect(ws1.url).toContain("access_token=auth_tokens%2Fephemeral_token_v1");
    if (ws1.onmessage) ws1.onmessage({ data: JSON.stringify({ goAway: { timeLeft: 3000 } }) });
    await vi.advanceTimersByTimeAsync(1500);
    expect(tokenProvider).toHaveBeenCalledTimes(2);
    expect(bridge.apiKey).toBe("auth_tokens/ephemeral_token_v2");
    expect(MockWebSocket.instances.length).toBe(2);
  });
