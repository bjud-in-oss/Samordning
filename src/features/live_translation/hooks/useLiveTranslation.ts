...
export function useLiveTranslation() {
  ...
  const startTranslation = useCallback(async () => {
    ...
    const dynamicTokenProvider = async (): Promise<string> => {
      const tokenRes = await fetch("/api/translation/token");
      if (!tokenRes.ok) {
        const errData = await tokenRes.json().catch(() => ({}));
        const errMsg = errData.error || "Ingen giltig Gemini API-nyckel/token tillgänglig.";
        throw new Error(errMsg);
      }
      const tokenData = await tokenRes.json();
      if (!tokenData?.token) {
        throw new Error("Ingen giltig sessionstoken mottogs från tokenservern.");
      }
    ...
    if (!activeApiKey || activeApiKey === "demo_key") {
      try {
        activeApiKey = await dynamicTokenProvider();
      } catch (tokenErr) {
        console.warn("[LiveTranslation] Token-hämtning misslyckades:", tokenErr);
        if (typeof process !== "undefined" && process.env.NODE_ENV === "test") {
          activeApiKey = "mock_test_key";
        } else {
          setError(tokenErr instanceof Error ? tokenErr.message : "Kunde inte ansluta till tokenservern för realtidstolkning.");
          setStatus("error");
          return;
        }
      }
    ...
    if (!activeApiKey || activeApiKey === "demo_key") {
      setError("Ingen giltig Gemini API-nyckel eller ephemeral token är konfigurerad.");
      setStatus("error");
      return;
    }
    ...
    try {
      ...
      const orchestrator = new MultiBridgeOrchestrator(activeApiKey, {
