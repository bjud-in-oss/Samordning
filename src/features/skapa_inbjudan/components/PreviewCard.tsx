...
interface PreviewCardProps {
  ...
  onToggleConsent?: (confirmed: boolean) => void;
  isAdmin?: boolean;
...
export function PreviewCard({
  ...
  return (
    <div className="bg-white rounded-3xl p-6 border border-brand-ink/10 shadow-md space-y-4 relative overflow-hidden text-left">
      ...
      {/* Integritetsbekräftelse - Formell och tydlig sektion */}
      <div className="pt-3 border-t border-brand-ink/10 space-y-2.5">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-sky-100 text-sky-800">
            <ShieldCheck size={16} />
          </div>
          <div>
            <h4 className="font-serif text-sm font-bold text-brand-ink leading-tight">
              Integritetsbekräftelse
            </h4>
            <span className="font-mono text-[9px] uppercase tracking-wider text-brand-ink/60 block font-bold">
              Skydd av personuppgifter
            </span>
          </div>
        </div>

        <p className="text-[11px] text-brand-ink/75 leading-relaxed font-light">
        ...
        <label className={`flex items-start gap-3 p-3 rounded-2xl border transition-all cursor-pointer ${
          consentConfirmed 
            ...
            : "bg-brand-paper/70 border-brand-ink/15 hover:border-brand-accent/30"
        }`}>
          <input
            type="checkbox"
            checked={consentConfirmed}
            onChange={e => onToggleConsent?.(e.target.checked)}
            className="mt-0.5 rounded border-brand-ink/30 text-brand-accent focus:ring-brand-accent shrink-0 cursor-pointer"
          />
      ...
      <div className="pt-3 border-t border-brand-ink/10 flex items-center justify-between gap-3">
        ...
        {onSend && (
          <button
            ...
            title={!consentConfirmed ? "Kryssa i integritetsbekräftelsen för att fortsätta" : undefined}
          ...
          >
            ...
            <span>
              {sending 
                ? (isAdmin ? "Publicerar direkt..." : "Granskar & skickar...") 
                : (isAdmin ? "Publicera direkt" : "Ge en inbjudan")}
            </span>
          </button>
        )}
      </div>
