...
export function setupRoutes(app: express.Express) {
  ...
  // Check device pairing status endpoints
  const checkPairingHandler = async (req: express.Request, res: express.Response) => {
    const token = String(req.query.token || "").trim();
    if (!token) return res.json({ active: false, status: "unpaired", paired: false, verified: false });

    // 1. Snabbkoll direkt i minnet
    if (pairedDevices.has(token) || pairedDevices.has(token.toLowerCase())) {
      return res.json({ active: true, status: "approved", paired: true, verified: true });
    }

    // 2. Fallback till Firestore via Admin-SDK
    ...
    if (db) {
      try {
        ...
        if (found) return res.json({ active: true, status: "approved", paired: true, verified: true });
      } catch (err) { console.warn(err); }
    }

    return res.json({ active: false, status: "unpaired", paired: false, verified: false });
  };

  app.get("/api/admin/check-pairing", checkPairingHandler);
  app.get("/api/check-pairing", checkPairingHandler);
  ...
  // Admin members & moderation routes
  setupAdminMemberRoutes(app);

  // Realtime Live Translation ephemeral token endpoint
  app.get("/api/translation/token", async (_req, res) => {
    try {
      const apiKey = process.env.GEMINI_API_KEY;
      if (!apiKey) {
        return res.status(503).json({
          error: "Ingen giltig Gemini API-nyckel konfigurerad på servern (GEMINI_API_KEY saknas)."
        });
      }
      ...
      console.error("[TranslationToken] Fel vid skapande av ephemeral token:", err);
      const msg = err instanceof Error ? err.message : "Internt fel vid tokenskapande";
