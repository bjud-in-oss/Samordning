...
export async function handleIncomingSms(req: Request, res: Response) {
  ...
  const loginMatch = trimmedText.match(/[\.#]Logga-in[-:\s]+([a-zA-Z0-9]+)/i);
  if (loginMatch) {
    const token = loginMatch[1];
    if (token) {
      pairDeviceToken(token);
      
      // Vi uppdaterar även loggen på baksidan till det nya språket
      addSimLog("system", `INLOGGNING: Skärm med kod ${token} låstes upp av ${sender}.`);
      
      const replyMessage = `Varmt tack för ditt engagemang och din tjänst idag! Om du nu återvänder till webbsidan, kan du se att du blivit inloggad.\n\nDu kan nu:\n• Godkänna andras förfrågningar om att bjuda in församlingen.\n• Välja och ändra språk för live-översättningen.\n\n...för att hjälpa fler att Vara en vän, Få näring av Guds ord och Hjälpa andra.`;
      
      const pairResp = { success: true, message: replyMessage };
